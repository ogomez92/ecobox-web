/**
 * The Anthropic API key used by the video description feature.
 *
 * Two sources, in this order:
 *  1. the `ai_credentials` row the user saved through the key dialog,
 *  2. `ANTHROPIC_API_KEY` from the environment — `.env` here, systemd's
 *     `EnvironmentFile=` — so a deploy can ship a working key with no UI step.
 *
...
/** Anthropic keys are `sk-ant-` followed by a long URL-safe tail; anything else is a paste error. */
...
function storedKey(): string {
	try {
		...
		console.error('Anthropic key read failed:', err);
		return '';
	}
}
