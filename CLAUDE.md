...
If you touched the bundled ELF engine (`elf/`) or Piper engine (`piper/`), include
them in the chown (`chown -R ecobox:ecobox build .svelte-kit elf piper`). The
`eci_synth` binary needs rebuilding (`cd elf/src && make`) after a fresh checkout on
a new machine, a CPU-arch change, or any change to `eci_synth.c` / the `eci/` wrapper
(e.g. the voice-parameter flags) — not on ordinary TS source edits. The Piper binary
is a prebuilt vendored release (no build step); re-download it for a new CPU arch (see
the Piper section below). Imported Piper voices live under `data/` (not the bundle),
so they survive deploys and are never chowned with the engine.
...
- **Piper (local engine)** (`$server/services/tts/piper.ts`): a second keyless, fully on-device service — but **neural** (VITS), not formant like ELF. ecobox vendors only the engine under `piper/` (the prebuilt `rhasspy/piper` Linux release: the `piper` binary + bundled `libonnxruntime`/`libpiper_phonemize`/`libespeak-ng` and `espeak-ng-data`; rpath `$ORIGIN`, with `LD_LIBRARY_PATH` set as a backstop). It ships **no voices** — the user **imports** them in Settings. Each voice is a `<stem>.onnx` model + `<stem>.onnx.json` config pair; uploads go through `POST /api/tts/piper/voices` (multipart, `config.body.maxSize` 1 GB, CSRF-guarded like any form POST) and are stored under **`PIPER_VOICES_DIR`** (`.env`, default `<dataDir>/piper-voices` — i.e. next to the db, **outside** the bundle so they survive deploys). `GET`/`DELETE` on that route list/remove imports; the engine bundle dir is `PIPER_DIR` (default `<cwd>/piper`). The adapter spawns `piper -q -m … -c … -f - --espeak_data …` per unit (single-line text on stdin → WAV on stdout; multi-speaker via `-s`), then pipes through **ffmpeg** to MP3 like ELF. `piperVoices()` enumerates `data/piper-voices/*.onnx` that have a readable `.onnx.json` (one voice per single-speaker model, one per speaker for multi-speaker → id `"<stem>"` or `"<stem>#<id>"`); `lang` comes from the config's `language.code` (`en_US`→`en-US`). **Unlike ELF, rate is NOT baked in** — Piper is neural and time-stretches cleanly, so it uses the **default audio path** (reader sets `<audio>.playbackRate`, `liveRate=true`, rate **not** in the cache key); the reader/`AudioEngine` need no Piper special-casing (everything keys off `service !== 'elf'`). No voice-param knobs and **no new DB columns** — voice selection reuses `tts_credentials.voiceId`. **`/api/tts/voices` does NOT cache `piper`** (its list is mutable at runtime; other services keep the 5-min cache). Needs system `ffmpeg`. **Engine source / updates:** the bundle is the prebuilt `rhasspy/piper` release (https://github.com/rhasspy/piper/releases) pinned at tag **`2023.11.14-2`**, asset `piper_linux_x86_64.tar.gz` (swap `x86_64`→`aarch64`/`armv7l` for other CPU arches), committed to git like `elf/` so a fresh clone deploys as-is. Heads-up before chasing updates: that repo is effectively in **maintenance** (no binary release since Nov 2023); active development moved to `OHF-Voice/piper1-gpl`, but that ships **Python wheels** (`pip install piper-tts`), **not** the standalone C++ binary this adapter spawns — so it is *not* a drop-in and would require reworking the adapter around a Python process. Voice models are versioned independently and stay forward-compatible, so a stale engine keeps working with newly imported voices. To update the engine binary (when a compatible release appears):

```bash
cd /home/ecobox
curl -sL -o /tmp/piper.tar.gz \
  "https://github.com/rhasspy/piper/releases/download/<TAG>/piper_linux_x86_64.tar.gz"
rm -rf piper && tar xzf /tmp/piper.tar.gz   # extracts to ./piper/ (binary + libs + espeak-ng-data)
chown -R ecobox:ecobox piper                # the service runs as User=ecobox
systemctl restart ecobox
git add piper/ && git commit -m "bump Piper engine to <TAG>"
